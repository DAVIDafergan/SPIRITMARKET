...
// ייצוא רשימת הקטגוריות (חשוב שנשאר)
...
const CreateListing: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. קבלת ניתוח AI עדכני
      const analysis = await Api.simulateVertexAIPrediction(file);
      
      // 2. שליחת כל הנתונים באובייקט אחד מאוחד (התיקון הקריטי)
      await Api.createListing({
        ...
        aiData: analysis // <--- הנה השינוי: aiData בתוך האובייקט הראשי
      });
