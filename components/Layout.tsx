...
import { Wine, PlusCircle, User as UserIcon, Menu, X, Globe, LogOut, Home as HomeIcon } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // פונקציה לעיגול שם המשתמש - תוקנה למניעת קריסה אם אין שם
  const UserAvatar = () => (
    <Link 
      to="/account" 
      title={t?.nav?.myAccount || 'My Account'} 
      className="w-10 h-10 rounded-full bg-gold-200 text-gold-900 flex items-center justify-center font-bold text-lg border-2 border-gold-400 hover:scale-105 transition-transform"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <nav className="hidden md:flex items-center gap-8">
              <Link to="/" className={isActive('/')}>{t?.nav?.market || 'Market'}</Link>
              {user?.isAdmin && (
                <Link to="/admin" className={isActive('/admin')}>{t?.nav?.moderation || 'Moderation'}</Link>
              )}
              {/* קישור לדף ניהול ליסטינג אישי */}
              {user && (
          ...
          <div className="md:hidden bg-charcoal-900 border-b border-gold-900/20 p-4 space-y-4">
            {user && (
              <div className="pb-4 border-b border-charcoal-800 flex items-center justify-between">
                 <span className="font-semibold text-gray-200">{t?.nav?.welcome || 'Welcome'}, {user.name}</span>
                 <button onClick={() => {logout(); setIsMenuOpen(false);}} className="text-sm text-red-500">{t?.nav?.logout || 'Logout'}</button>
              </div>
            )}
            
            <Link to="/" className="flex items-center gap-3 py-2 text-gray-300" onClick={() => setIsMenuOpen(false)}>
              <HomeIcon size={18} /> {t?.nav?.market || 'Market'}
            </Link>

            {/* קישור לאיזור האישי - מובייל */}
            {user && (
              <>
                <Link to="/account" className="flex items-center gap-3 py-2 text-gray-300" onClick={() => setIsMenuOpen(false)}>
                  <UserIcon size={18} /> {t?.nav?.myAccount || 'My Account'}
                </Link>
                {/* קישור לניהול הליסטינג - מובייל */}
                <Link to="/my-listings" className="flex items-center gap-3 py-2 text-gray-300" onClick={() => setIsMenuOpen(false)}>
                  <PlusCircle size={18} /> {t?.nav?.myListings || 'My Listings'}
                </Link>
              </>
            )}

            {user?.isAdmin && (
               <Link to="/admin" className="flex items-center gap-3 py-2 text-gray-300" onClick={() => setIsMenuOpen(false)}>
                  <Wine size={18} /> {t?.nav?.moderation || 'Moderation'}
               </Link>
            )}
            ...
            {!user && (
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-charcoal-800">
                <Link to="/login" className="text-center py-2 text-gray-300 border border-charcoal-700 rounded-lg" onClick={() => setIsMenuOpen(false)}>{t?.nav?.login || 'Login'}</Link>
                <Link to="/register" className="text-center py-2 bg-gold-500 text-charcoal-950 rounded-lg font-bold" onClick={() => setIsMenuOpen(false)}>{t?.nav?.register || 'Register'}</Link>
              </div>
            )}
          </div>
        )}
      ...
      <footer className="bg-charcoal-950 text-gray-400 py-12 border-t border-gold-900/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="space-y-4">
            ...
            {/* HERE WAS THE ERROR - FIXED WITH OPTIONAL CHAINING */}
            <p className="text-sm text-gray-500">
              {t?.footer?.description || 'The safest marketplace for buying and selling rare spirits and fine wines. Powered by advanced AI authentication.'}
            </p>
          </div>
